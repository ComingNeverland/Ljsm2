#!/usr/bin/perl
# noCVS: $Id: ljsm.pl,v 3.0 2011/04/09 15:19 ELBY Exp $
# Updater: ELBY
...
#perl2exe_info ProductVersion=3.0.0.0
#perl2exe_info FileVersion=3.0.0.0
...
#	LOCAL_CHARSET	=> 'windows-1251', # windows cyrillic
	...
	CLIENT			=> 'Perl-ljsm/3.0; alpha',
	CVSVERSION		=> '$Revision: 3.0 $', # don't touch this
...
our ($opt_r, $opt_m, $opt_a, $opt_c, $opt_O, $opt_i, $opt_I, $opt_u, $opt_U, $opt_x, $opt_t, $opt_p, $opt_d, $opt_T, $opt_v, $opt_g, $opt_q, $opt_L);
...
getopts('rmacxtOIUvgqLu:p:d:i:T:');
...
if(defined($opt_T) && ($opt_T+0))
{
		...
		logmsg("\n\nTimout changed from " . ($ua->timeout()) . " seconds to $t\n\n", 2);
		$ua->timeout($t);
}
...
logmsg("\n\nHTTP 1.0 protocol selected, expecting some traffic increase\n") if($opt_g);
...
foreach $user (@ARGV) { # for each user
	...
	if($opt_q)
	{
		create_sql($user)	or die("error creating SQL dump");
		
		#dump_sql($user, "Мусикака'one' ", "http://www.ru", "robots.txt");
		#exit;
	}
...
sub get_memos {
	...
	if ($content = get_page(BASE_URL . MEMO_SCRIPT . "?user=$user")) 
	{
		...
		foreach $link (&tiny_link_extor(\$content, 0)) {
			...
			# by ELBY
			push @keywords_extracted, $keyword;
			...
			if ($content = get_page($link)) {
				...
				foreach $link_post (&tiny_link_extor(\$content, 0)) {
					...
					print "\nAdded post $link_post / keyword $keyword\n";
					print "Totally " . @memos. " posts\n";
				}
				
				# by ELBY
				if(my $cnt=@keywords_extracted)
				{
					print "\nKeywords extracted ($cnt):" . join (", ", @keywords_extracted) . "\n\n";
				}

			} else { # error fetching list of posts

				# by ELBY
				push @failed_list, $link;
		...
		# by ELBY
		push @failed_list, BASE_URL . MEMO_SCRIPT . "?user=$user";
...
sub get_files {
	...
	foreach $post (@posts) {
		...
		my $curfile="$dir/$fname";

		# by ELBY			
		if($opt_q && ($post->{'type'} eq 'memo'))
		{
			# add SQL dump entry, if required
				
			dump_sql($user, $post->{'keyword'}, $post->{'link'}, $curfile);
			
			next	if($obtained_list{$curfile} && (-s $curfile) && (-r $curfile));
		}

		print "\nObtain " . $post->{'link'} . "\n";

		my $diskfile_loaded=0;
		if (-s "$curfile") 
		{
			if ($opt_O) 
			{
				logmsg("!! overwriting $curfile\n", 2);
			} 
			elsif ($opt_r) 
			{
				logmsg("-r: skipping $curfile\n", 2);

				next if(! $opt_L);

				# by ELBY
				$content=read_diskfile($curfile);
				my $len=length($content);
				
				if(! $len)
				{
					logmsg("unable to load diskfile $curfile\n", 0);
					next;
				}
				else
				{
					logmsg("$len bytes of $curfile loaded from cache\n");
					$diskfile_loaded=1;
				}
		...
		# by ELBY
		$content = get_page($post->{'link'}, 0, $need_threader)	if(! $diskfile_loaded);
		
		if($content)
		{
			$obtained_list{$curfile}=1;
			
			# by ELBY
			write_diskfile($curfile, \$content)	if(! $diskfile_loaded);
...
# by ELBY
=item get_diskfile_name(\$fname)

get a cached file name

=cut
sub get_diskfile_name
{
	my($fname) = @_;
...
# by ELBY
=item read_diskfile(\$fname)

load a cache file if it already exist
...
sub read_diskfile 
{
	...
	$fname	= get_diskfile_name ($fname);
	return undef	if((! -r $fname) && (! -s $fname));

	open LOCFILE, "<$fname" or return undef;
	binmode LOCFILE;
  read (LOCFILE, $str, -s LOCFILE);
	close LOCFILE;
	
	# Decompress before use
	return Compress::Zlib::memGunzip($str);

}


# by ELBY
=item write_diskfile(\$fname, \$content)
...
sub write_diskfile
{
	...
	$fname	= get_diskfile_name ($fname);
	#return undef	if(! -w $fname);

	# Compress before store
	my $len=length($content);
	#logmsg("\nCompressing $len bytes");
	
	my $str=Compress::Zlib::memGzip($content);
	if(! length($str))
	{
		logmsg("\nGzip compress of $fname returned 0 bytes\n",0);
		return undef;
	}
...
sub create_sql
{
	...
	my $where=LOCAL_DIR . "$user/memories";
	mkpath($where, 0755);

	if( open(SQL, ">${where}/keywords.sql") )
	{
print SQL <<END;
# MySQL part
CREATE TABLE if not exists keyword (keyword_id bigint not NULL auto_increment PRIMARY KEY, keyword varchar(255) not null unique) CHARACTER SET=utf8;
CREATE TABLE if not exists url (url_id bigint not NULL auto_increment PRIMARY KEY, file text not null, url text not null) CHARACTER SET=utf8;
CREATE TABLE if not exists memories (user varchar (232) not null, url_id bigint not NULL, keyword_id bigint not NULL) CHARACTER SET=utf8;
ALTER IGNORE TABLE memories ADD unique chck(user, url_id, keyword_id), ADD index user(user), ADD index url_id(url_id), ADD index keyword_id(keyword_id);
...
# by ELBY
=item dump_sql(\$user, \$keyword, \$url, \$fname)

add a 'memories' keyword entry into a SQL dump

=cut
sub dump_sql
{
	my($user,$keyword,$url,$fname) = @_;
	return unless $opt_q;
	
	my $fpath=LOCAL_DIR . "$user/memories/keywords.sql";
	
	if( open(SQL, ">>$fpath") )
	{
		$user = quote_sql($user);
		$keyword = quote_sql($keyword);
		$url		= quote_sql($url);
		$fname	= quote_sql($fname);
		
		print SQL "INSERT IGNORE INTO keyword SET keyword='$keyword';\n";
		print SQL "INSERT IGNORE INTO url SET file='$fname', url='$url';\n";
		print SQL "INSERT IGNORE INTO memories SELECT '$user', url_id, keyword_id FROM keyword LEFT JOIN url ON (url.url='$url') WHERE keyword='$keyword';\n\n";
		
		close SQL;
		
		logmsg("*** url $url marked with keyword '$keyword'\n", 0);
	}
	else
	{
		logmsg("\nUnable to write dump $fpath\n");
	}
}
...
sub get_page {
	...
	# Remove ads
	if($opt_v && (($url =~ /^http(s){0,1}:\/\/sol\.adbureau\.net/i) || ($url =~ /^http(s){0,1}:\/\/www\.tns-counter\.ru/i)) || 
								($url =~ /^http(s){0,1}:\/\/ad\.doubleclick\.net/i))
	{
	    logmsg("AD skipped: $url\n", 0);
	    print LF "HUH\n";
	    return undef;
	}
	...
	foreach (1 .. MAX_TRIES) {
		...
		if ($res->is_success) {
			...
			print "\nPage ok\n\n";
			my $tmx=$res->content;
			print "\n". length($tmx) . " bytes get\n\n";
			
			if($res->content_encoding && ($res->content_encoding =~ /gzip/))
			{
			    my $tmx=Compress::Zlib::memGunzip($res->content);
			    print "\nGunzipped: ". length($tmx) . "\n\n";
			    #printf "\ngzerrno=$gzerrno\n";
...
3.0 options
===========
 -T value = set HTTP request timout, in seconds
 -v = ignore SUP advertising (if already blocked on firewall)
 -g = use HTTP 1.0 requests (does not use Gzip compression)
 -q = save SQL dump for "memories" keywords
 -l = use locally cached file copies (*experimental*)
...
END {
	...
	if ((DEBUG_LEVEL > 0) && (scalar keys %stat)) {
		...
		if(my $cnt= @failed_list)
		{
			print "\nThese url's ($cnt) failed:\n\n";
	
			foreach my $url ( @failed_list )
			{	
				print "  $url\n";
			}
		}
